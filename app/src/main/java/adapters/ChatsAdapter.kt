...
class ChatsAdapter(mContext: Context, mChatFragment: ChatFragment, width: Int, chats: LinkedHashMap<String, ChatsModel>,
    ...
    override fun onBindViewHolder(holder: ViewHolder?, position: Int) {
        ...
        var otherUserId = mPrivateChat!!.opponent_user_id
...
//        }
        ...
        if (!mPrivateChat.last_message.equals(Constants.DEFAULT_MESSAGE_REGEX)) {
            try {
                ...
                cal.setTimeInMillis(Constants.getLocalTime(mPrivateChat.last_message_time))
                val today = Calendar.getInstance()
                if (date_format.format(today.time) == date_format.format(cal.getTime())) {
                    // today
