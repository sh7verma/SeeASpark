...
public class PickerManager {
  ...
  private int maxCount = Constants.DEFAULT_MAX_COUNT;
  ...
  private int theme = R.style.MainActionBarTheme;
  ...
  public void add(String path, int type) {
    if (path != null && shouldAdd()) {
      if (!mediaFiles.contains(path) && type == Constants.FILE_TYPE_MEDIA) {
        mediaFiles.add(path);
      } else if (!docFiles.contains(path) && type == Constants.FILE_TYPE_DOCUMENT) {
        docFiles.add(path);
      } else {
        return;
      }
  ...
  public void remove(String path, int type) {
    if ((type == Constants.FILE_TYPE_MEDIA) && mediaFiles.contains(path)) {
      mediaFiles.remove(path);
    } else if (type == Constants.FILE_TYPE_DOCUMENT) {
  ...
  public void addDocTypes() {
    String[] pdfs = { "pdf" };
    fileTypes.add(new FileType(Constants.PDF, pdfs, R.drawable.icon_file_pdf));

    String[] docs = { "doc", "docx", "dot", "dotx" };
    fileTypes.add(new FileType(Constants.DOC, docs, R.drawable.icon_file_doc));

//    String[] ppts = { "ppt", "pptx" };
//    fileTypes.add(new FileType(Constants.PPT, ppts, R.drawable.icon_file_ppt));
//
//    String[] xlss = { "xls", "xlsx" };
//    fileTypes.add(new FileType(Constants.XLS, xlss, R.drawable.icon_file_xls));

    String[] txts = { "txt" };
    fileTypes.add(new FileType(Constants.TXT, txts, R.drawable.icon_file_unknown));
  }
