...
public class FilePickerBuilder {
  ...
  public FilePickerBuilder setSelectedFiles(ArrayList<String> selectedPhotos) {
    mPickerOptionsBundle.putStringArrayList("SELECTED_DOCS", selectedPhotos);
    return this;
  }
...
//  }
  ...
  public void pickFile(Activity context) {
    mPickerOptionsBundle.putInt(Constants.EXTRA_PICKER_TYPE, Constants.DOC_PICKER);
    start(context, Constants.REQUEST_CODE_DOC);
  }
...
//  }
  ...
  private void start(Activity context, int requestCode) {
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
      if (ContextCompat.checkSelfPermission(context, Constants.PERMISSIONS_FILE_PICKER)
    ...
    Intent intent = new Intent(context, DocPickerActivity.class);
