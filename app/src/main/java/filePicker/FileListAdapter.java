...
public class FileListAdapter extends SelectableAdapter<FileListAdapter.FileViewHolder, Document>
  ...
  public void onBindViewHolder(final FileViewHolder holder, int position) {
    ...
    holder.itemView.setBackgroundResource(
        isSelected(document) ? R.color.askmo_titlecolor : android.R.color.white);
    ...
    holder.checkBox.setOnCheckedChangeListener(new SmoothCheckBox.OnCheckedChangeListener() {
      ...
      public void onCheckedChanged(SmoothCheckBox checkBox, boolean isChecked) {
        ...
        holder.itemView.setBackgroundResource(isChecked ? R.color.askmo_titlecolor : android.R.color.white);
      }
    });
  }
  ...
  private void onItemClicked(Document document, FileViewHolder holder) {
    if (PickerManager.getInstance().getMaxCount() == 1) {
      PickerManager.getInstance().add(document.getPath(), Constants.FILE_TYPE_DOCUMENT);
    } else {
      if (holder.checkBox.isChecked()) {
        PickerManager.getInstance().remove(document.getPath(), Constants.FILE_TYPE_DOCUMENT);
